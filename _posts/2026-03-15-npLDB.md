...
We consider the following setting: at one integer cutoff (for ease, I use 1.00), fix a discontinuity 
size and latent supply curve slope. The researcher attempts to estimate the inattention parameter
\\(\theta\\) using two different methods: 
1. `rdrobust` **discontinuity**: Using the `rdrobust` package from Calonico, Catteneo, and Titiunik, 
estimate the discontinuity at the integer using local polynomial regression (local linear regression by default)
...
opt for MSE-optimal bandwidth selection for simplicity. 
2. **Discontinuous linear splines via kink-discontinuity regression**: As in the big data economist's 
siren setting from above, instead use kink-discontinuity (linear) regression to estiamte the 
discontinuity at the integer cutoff: 
$$
y_i \sim \left(1, \; P_i, \; \boldsymbol{1}\{P_i = 1\}, \; \boldsymbol{1}\{P_i = 1\}P_i\right).
$$

I assume the researcher has access to experimental data analogous to List et al., which 
leverages a random multiplier instrument \\(M\\) to exogenously vary the price of rides 
offered to riders (i.e., \\(p_{\mathrm{final}} = Mp_{\mathrm{original}}\\)), so we 
use two-stage least squares to reliably estimate the slope of the latent curve of interest
(for List and coauthors, that is the demand curve; for my investigation, it is the 
supply curve). The method is still valid in the supply-side LDB setting with a different 
interpretation (e.g., the random multiplier now varies the price of rides displayed to drivers). 
